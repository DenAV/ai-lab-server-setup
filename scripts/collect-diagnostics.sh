...
# Traefik certs info (not the certs themselves)
collect "traefik-certs" "docker exec traefik cat /certs/acme.json 2>/dev/null | python3 -c \"
import json, sys
try:
...
except: print('No certificates or parse error')
\" 2>/dev/null"
