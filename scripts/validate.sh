...
if [ -f "${PROJECT_DIR}/.env" ] && docker compose -f "${COMPOSE_FILE}" ps --quiet 2>/dev/null | grep -q .; then
  ...
  check "Langfuse API"        "docker exec langfuse node -e \"require('http').get('http://localhost:3000/',r=>{process.exit(r.statusCode<400?0:1)}).on('error',()=>process.exit(1))\" 2>/dev/null"
  check "Dify API"            "docker exec dify-api python3 -c \"import urllib.request; urllib.request.urlopen('http://localhost:5001/')\" 2>/dev/null"
fi
